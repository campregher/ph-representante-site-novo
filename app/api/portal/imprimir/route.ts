...
export async function GET(request: Request) {
  ...
  const { data: clienteRow } = await supabase
    ...
    .eq("user_id", user.id)
    .single();
  if (!clienteRow) return new NextResponse("Cliente não encontrado", { status: 403 });

  const { searchParams } = new URL(request.url);
  const ids = searchParams.get("ids")?.split(",").filter(Boolean).slice(0, 50) ?? [];
  const idSingle = searchParams.get("id");
  if (idSingle && !ids.length) ids.push(idSingle);
  ...
  const pages = orcamentos.map((o) => {
    ...
    const clienteData = o.clientes as Record<string, string | null | undefined> | null;
    ...
    const clienteHtml = clienteData ? `
      <div style="font-size:9.5pt;font-weight:700;color:#111;margin-bottom:2px">${clienteData.razao_social}</div>
      ${clienteData.nome_fantasia ? `<div style="font-size:7.5pt;color:#4b5563;margin-bottom:3px">${clienteData.nome_fantasia}</div>` : ""}
      ${clienteData.cnpj     ? `<div style="font-size:7pt;color:#6b7280;margin-bottom:2px">CNPJ: ${clienteData.cnpj}</div>` : ""}
      ${clienteData.email    ? `<div style="font-size:7pt;color:#6b7280;margin-bottom:2px">${clienteData.email}</div>` : ""}
      ${clienteData.whatsapp ? `<div style="font-size:7pt;color:#6b7280;margin-bottom:2px">${clienteData.whatsapp}</div>` : ""}
      ${addr(clienteData)    ? `<div style="font-size:7pt;color:#6b7280">${addr(clienteData)}</div>` : ""}
    ` : "<div style='font-size:7.5pt;color:#9ca3af'>—</div>";
