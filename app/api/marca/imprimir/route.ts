...
export async function GET(request: Request) {
  ...
  const { data: orcamentos } = await db
    .from("orcamentos")
    .select("*, orcamento_itens(*), orcamento_etiquetas(*), clientes(razao_social, nome_fantasia, cnpj, email, whatsapp, logradouro, numero, bairro, cidade, estado, cep)")
    .in("id", ids)
  ...
  // Marca como impresso (fire-and-forget)
  void Promise.resolve(
    db.from("orcamentos")
      .update({ impresso_at: new Date().toISOString() })
      .in("id", ids)
      .eq("marca", ctx.marcaSlug)
  ).catch(() => {});

  const { data: marcaData } = await db
    .from("marcas")
    .select("slug, name, razao_social, cnpj, email, telefone, logradouro, numero, bairro, cidade, estado")
...
<div style="width:210mm;min-height:297mm;padding:12mm 16mm 14mm;background:#fff;color:#1a1a1a;font-family:Arial,Helvetica,sans-serif;page-break-after:always;box-sizing:border-box;position:relative">
  ...
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px">
    <div>
      ...
      ${marcaData?.razao_social ? `<div style="font-size:7.5pt;color:#4b5563;margin-top:2px">${marcaData.razao_social}</div>` : ""}
      ${marcaData?.cnpj         ? `<div style="font-size:7pt;color:#9ca3af;margin-top:1px">CNPJ: ${marcaData.cnpj}</div>` : ""}
      ${marcaData?.email        ? `<div style="font-size:7pt;color:#9ca3af;margin-top:1px">${marcaData.email}</div>` : ""}
      ${marcaData?.telefone     ? `<div style="font-size:7pt;color:#9ca3af;margin-top:1px">${marcaData.telefone}</div>` : ""}
      ${addr(marcaData as Record<string, string | null>) ? `<div style="font-size:7pt;color:#9ca3af;margin-top:1px">${addr(marcaData as Record<string, string | null>)}</div>` : ""}
    </div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;border:0.5pt solid #e5e7eb;border-radius:8px;overflow:hidden;margin-bottom:16px">
    <div style="padding:12px;background:#fafafa">
      ...
      ${marcaData?.razao_social ? `<div style="font-size:7.5pt;color:#4b5563;margin-bottom:3px">${marcaData.razao_social}</div>` : ""}
      ${marcaData?.cnpj   ? `<div style="font-size:7pt;color:#6b7280;margin-bottom:2px">CNPJ: ${marcaData.cnpj}</div>` : ""}
      ${marcaData?.email  ? `<div style="font-size:7pt;color:#6b7280;margin-bottom:2px">${marcaData.email}</div>` : ""}
      ${marcaData?.telefone ? `<div style="font-size:7pt;color:#6b7280;margin-bottom:2px">${marcaData.telefone}</div>` : ""}
      ${addr(marcaData as Record<string, string | null>) ? `<div style="font-size:7pt;color:#6b7280">${addr(marcaData as Record<string, string | null>)}</div>` : ""}
    </div>
...
<head>
  ...
  <title>Pedidos — ${marcaData?.name ?? ctx.marcaSlug}</title>
  <style>
